//! Jupyter notebook (.ipynb) format adapter: import and export for RelationalText documents.
//!
//! Format namespace: `org.jupyter.facet`
//!
//! WASM interface:
//!   alloc / dealloc / result_len — memory boilerplate (from wasm-format-adapter)
//!   import(ptr, len) -> ptr     — Jupyter notebook JSON (in "text" field) → DocumentJSON
//!   export(ptr, len) -> ptr     — DocumentJSON (Jupyter facets) → raw Jupyter notebook JSON

use relationaltext_core::{
    hir::{build_hir_from_doc, HirNode},
    serde_atproto, LexiconRegistry,
};
use relationaltext_wasm_format_adapter::{read_str, write_result};
use serde_json::{json, Value};
use std::sync::OnceLock;

const TYPE_ID: &str = "org.jupyter.facet";

// ─── Lexicon registry ─────────────────────────────────────────────────────────

const LEXICON_JSON: &[u8] = include_bytes!("../../../formats/org.jupyter/jupyter.lexicon.json");

static REGISTRY: OnceLock<LexiconRegistry> = OnceLock::new();

fn registry() -> &'static LexiconRegistry {
    REGISTRY.get_or_init(|| {
        let lexicon: Value = serde_json::from_slice(LEXICON_JSON).unwrap_or(Value::Null);
        let empty = Vec::new();
        let features = lexicon["features"].as_array().unwrap_or(&empty).to_vec();
        let mut r = LexiconRegistry::new();
        let _ = r.register_from_json_array(&features);
        r
    })
}

// ─── WASM exports ─────────────────────────────────────────────────────────────

#[no_mangle]
pub extern "C" fn import(ptr: *mut u8, len: i32) -> *mut u8 {
    let input = unsafe { read_str(ptr, len) };
    let result = do_import(input);
    write_result(result)
}

#[no_mangle]
pub extern "C" fn export(ptr: *mut u8, len: i32) -> *mut u8 {
    let input = unsafe { read_str(ptr, len) };
    let result = do_export(input);
    write_result(result)
}

// ─── Import: Jupyter notebook JSON → DocumentJSON ─────────────────────────────

struct ImportState {
    text: String,
    facets: Vec<Value>,
}

impl ImportState {
    fn new() -> Self {
        Self {
            text: String::new(),
            facets: Vec::new(),
        }
    }
}

fn do_import(raw: &str) -> String {
    // Input is DocumentJSON where text = Jupyter notebook JSON string
    let outer: Value = serde_json::from_str(raw).unwrap_or(Value::Null);
    let nb_str = outer["text"].as_str().unwrap_or(raw);

    let notebook: Value = serde_json::from_str(nb_str).unwrap_or(Value::Null);

    // Determine the default language from kernelspec or language_info
    let default_language = notebook["metadata"]["kernelspec"]["language"]
        .as_str()
        .or_else(|| notebook["metadata"]["language_info"]["name"].as_str())
        .unwrap_or("python")
        .to_owned();

    let empty = Vec::new();
    let cells = notebook["cells"].as_array().unwrap_or(&empty);

    let mut state = ImportState::new();

    for cell in cells {
        let cell_type = cell["cell_type"].as_str().unwrap_or("markdown");

        // source can be a string or an array of strings
        let source = if cell["source"].is_array() {
            cell["source"]
                .as_array()
                .unwrap_or(&empty)
                .iter()
                .filter_map(|v| v.as_str())
                .collect::<Vec<_>>()
                .join("")
        } else {
            cell["source"].as_str().unwrap_or("").to_owned()
        };

        if cell_type == "code" {
            let mut attrs = json!({ "language": default_language });
            if let Some(id) = cell["id"].as_str() {
                attrs["id"] = Value::String(id.to_owned());
            }
            open_block(&mut state, "code", attrs);
            state.text.push_str(&source);
        } else {
            // 'markdown' or 'raw' — treat as markdown-cell
            let mut attrs = json!({});
            if let Some(id) = cell["id"].as_str() {
                attrs["id"] = Value::String(id.to_owned());
            }
            open_block(&mut state, "markdown", attrs);
            state.text.push_str(&source);
        }
    }

    json!({ "text": state.text, "facets": state.facets }).to_string()
}

fn open_block(state: &mut ImportState, name: &str, attrs: Value) {
    let marker_char = if state.text.is_empty() {
        '\u{FFFC}'
    } else {
        '\n'
    };
    let marker_start = state.text.len();
    state.text.push(marker_char);
    let marker_end = state.text.len();

    let has_attrs = attrs.as_object().map(|o| !o.is_empty()).unwrap_or(false);
    let mut feature = json!({
        "$type": TYPE_ID,
        "name": name,
    });
    if has_attrs {
        feature["attrs"] = attrs;
    }
    state.facets.push(json!({
        "index": { "byteStart": marker_start, "byteEnd": marker_end },
        "features": [feature],
    }));
}

// ─── Export: DocumentJSON → Jupyter notebook JSON ─────────────────────────────

fn capitalize(s: &str) -> String {
    let mut chars = s.chars();
    match chars.next() {
        None => String::new(),
        Some(c) => c.to_uppercase().collect::<String>() + chars.as_str(),
    }
}

fn do_export(doc_json: &str) -> String {
    let doc = match serde_atproto::from_json(doc_json) {
        Ok(d) => d,
        Err(_) => {
            return json!({
                "nbformat": 4,
                "nbformat_minor": 5,
                "metadata": {
                    "kernelspec": { "language": "python", "display_name": "Python", "name": "python" },
                    "language_info": { "name": "python" }
                },
                "cells": []
            }).to_string();
        }
    };

    let nodes = build_hir_from_doc(&doc, registry());
    let mut cells: Vec<Value> = Vec::new();
    let mut first_code_language: Option<String> = None;

    for node in &nodes {
        if let HirNode::Block {
            name,
            attrs,
            children,
        } = node
        {
            // Extract text content from children
            let text_content: String = children
                .iter()
                .filter_map(|c| {
                    if let HirNode::Text { content, .. } = c {
                        Some(content.as_str())
                    } else {
                        None
                    }
                })
                .collect();

            let cell_id = attrs.get("id").and_then(|v| v.as_str());

            if name == "markdown" {
                let mut cell = json!({
                    "cell_type": "markdown",
                    "source": [text_content],
                    "metadata": {}
                });
                if let Some(id) = cell_id {
                    cell["id"] = Value::String(id.to_owned());
                }
                cells.push(cell);
            } else if name == "code" {
                let cell_language = attrs
                    .get("language")
                    .and_then(|v| v.as_str())
                    .unwrap_or("python")
                    .to_owned();
                if first_code_language.is_none() {
                    first_code_language = Some(cell_language.clone());
                }
                let mut cell = json!({
                    "cell_type": "code",
                    "source": [text_content],
                    "metadata": {},
                    "execution_count": null,
                    "outputs": []
                });
                if let Some(id) = cell_id {
                    cell["id"] = Value::String(id.to_owned());
                }
                cells.push(cell);
            }
        }
    }

    let lang = first_code_language.unwrap_or_else(|| "python".to_owned());

    json!({
        "nbformat": 4,
        "nbformat_minor": 5,
        "metadata": {
            "kernelspec": {
                "language": lang,
                "display_name": capitalize(&lang),
                "name": lang
            },
            "language_info": { "name": lang }
        },
        "cells": cells
    })
    .to_string()
}
